    ...
    return `
      <div class="case-item" onclick="App.editCase(${c.id})" style="cursor:pointer;">
        ...
        <div style="margin-top:6px; display:flex; gap:8px; flex-wrap:wrap; font-size:12px;">
          <span class="case-tag ${(d.a||d.b||d.c) ? 'yes' : 'no'}">划扣：${(d.a?1:0)+(d.b?1:0)+(d.c?1:0)}/3 项已划扣</span>
        </div>
  ...
  showAddCaseModal(existing) {
    ...
    this.showModal(isEdit ? '编辑案例' : '添加每日案例', `
      ...
      <div style="display:flex; gap:8px; margin-bottom:12px;">
        <div class="chip ${(d.a&&d.b&&d.c)?'active':''}" id="caseDeductBtn" onclick="App._cycleDeductState(this)">${(d.a&&d.b&&d.c)?'✓ 全部已划扣 (3/3)':(d.a||d.b||d.c)?'⏳ 部分划扣 ('+((d.a?1:0)+(d.b?1:0)+(d.c?1:0))+'/3)':'✗ 未划扣 (0/3)'}</div>
      </div>
      <input type="hidden" id="deductState" value="${(d.a&&d.b&&d.c)?'all':(d.a||d.b||d.c)?'some':'none'}">
    ...
    `, () => {
      ...
      const ds = document.getElementById('deductState')?.value || 'none';
      const a = ds === 'all'; const b = ds === 'all'; const c = ds === 'all';
