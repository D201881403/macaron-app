...
const App = {
      ...
      <div class="card">
        ...
        ${(this.reminders || []).length === 0
          ...
          : (this.reminders || []).map(r => {
            const repeatLabel = { daily: '每天', weekday: '工作日', weekend: '周末', custom: '自定义' }[r.repeat] || '每天';
  ...
  clearData() {
    if (confirm('确定要清空所有数据吗？此操作不可恢复。')) {
      ['profile','todos','habits','schedules','diaries','expenses','notes','health','customModuleData','dailyPlans','weeklyReports','aiRules','focusGuided','reminders'].forEach(k => Store.remove(k));
      this.loadProfile();
      this.loadAllData();
      this.renderTabbar();
      this.navigate('settings');
...
END:VCALENDAR`;
  ...
  startReminderChecker() {
    ...
    this._reminderCheckTimer = setInterval(() => this.checkReminders(), 30000);
    this.checkReminders();
  },
  ...
  fireReminder(r) {
    if ('Notification' in window && Notification.permission === 'granted') {
      try {
        new Notification('马卡龙空间 - 提醒', { body: r.name, tag: 'reminder-' + r.id, vibrate: [200, 100, 200] });
      } catch(e) {}
    }
    ...
    overlay.innerHTML = `
      <div class="modal reminder-modal">
        <div style="text-align:center; padding: 20px 0;">
          <div style="font-size: 48px; margin-bottom: 12px; animation: bellShake 0.5s ease 3;">🔔</div>
          <div style="font-size: 20px; font-weight: 700; margin-bottom: 8px;">${r.name}</div>
          <div style="font-size: 14px; color: var(--text-light);">${r.time}</div>
        </div>
    ...
    if ('Notification' in window) {
      if (Notification.permission === 'default') {
        Notification.requestPermission().then(p => {
          this.toast(p === 'granted' ? '通知权限已开启' : '未开启通知权限，将使用页面弹窗提醒');
        });
      } else if (Notification.permission === 'granted') {
        this.toast('通知权限已开启');
      } else {
        this.toast('通知权限被拒绝，请在浏览器设置中开启');
      }
    } else {
      this.toast('当前浏览器不支持通知，将使用页面弹窗提醒');
    }
  },
  ...
  showReminderModal_edit(existing) {
    ...
    this.showModal(isEdit ? '编辑提醒' : '添加提醒', `
      ...
      <div style="display:flex; gap:8px; flex-wrap:wrap;">
        ...
        <div class="chip reminder-repeat ${r.repeat==='custom'?'active':''}" data-repeat="custom" onclick="App.selectReminderRepeat(this)">⚙️ 自定义</div>
      </div>
      <div class="field-label" id="customDaysLabel" style="display:${r.repeat==='custom'?'block':'none'};">选择日期</div>
      <div style="display:${r.repeat==='custom'?'flex':'none'}; gap:6px; flex-wrap:wrap;" id="customDaysContainer">
        ${['日','一','二','三','四','五','六'].map((d, i) => `
          <div class="chip reminder-custom-day ${(r.customDays||[]).includes(i)?'active':''}" data-day="${i}" onclick="App.toggleCustomDay(this)">${d}</div>
        `).join('')}
      </div>
    `, () => {
