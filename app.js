...
const App = {
  ...
  clearData() {
    if (confirm('确定要清空所有数据吗？此操作不可恢复。')) {
      ['profile','todos','habits','schedules','diaries','expenses','notes','health','customModuleData','dailyPlans','weeklyReports','aiRules','focusGuided','reminders','pwaDataChecked'].forEach(k => Store.remove(k));
      this.loadProfile();
      this.loadAllData();
      this.renderTabbar();
      this.navigate('settings');
  ...
  showCalendarExportModal() {
    this.showModal('同步到系统日历', `
      ...
      <button class="btn btn-primary" style="width:100%; margin-top:8px; background:#FF9800;" onclick="App.syncRemindersToCalendar()">🔔 同步提醒到系统日历（可后台响铃）</button>
    `, null);
    setTimeout(() => {
      const confirmBtn = document.getElementById('modalConfirm');
      const cancelBtn = document.getElementById('modalCancel');
...
END:VCALENDAR`;
  ...
  // 生成提醒专用的 .ics 文件（每个提醒一个带 VALARM 的事件）
  generateReminderICS() {
    const now = new Date();
...
END:VCALENDAR`;
  ...
  _getReminderSyncHTML() {
    ...
    return `
      <div style="margin-top:12px; padding:12px; background:#FFF3E0; border-radius:12px; text-align:left;">
        ...
        <button class="btn btn-primary" style="width:100%; background:#FF9800;" onclick="App.syncRemindersToCalendar()">
          📅 同步 ${count} 个提醒到系统日历
        </button>
        <div style="font-size:11px; color:#999; margin-top:6px; text-align:center;">
          下载后点击 .ics 文件 → 自动导入iPhone日历 → 到点响铃
        </div>
      </div>`;
  },
  ...
  // 播放提示音（使用 Web Audio API 生成铃声）
  _playAlertSound() {
    try {
      // 使用持久化的 AudioContext（在用户首次点击时已初始化）
      if (!this._audioCtx) {
    ...
    overlay.innerHTML = `
      <div class="modal reminder-modal">
        <div style="text-align:center; padding: 20px 0;">
          <div style="font-size: 56px; margin-bottom: 12px; animation: bellShake 0.4s ease 4;">🔔</div>
          <div style="font-size: 22px; font-weight: 700; margin-bottom: 6px;">${r.name}</div>
          <div style="font-size: 16px; color: var(--primary-dark); font-weight:600;">${r.time}</div>
        </div>
        <button class="btn btn-primary" style="width:100%; font-size:16px; padding:14px;" onclick="this.closest('.modal-overlay').remove();">我知道了</button>
      </div>
    `;
    document.body.appendChild(overlay);
