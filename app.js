...
END:VCALENDAR`;
  ...
  async restoreFromCloud(isFreshLogin) {
    try {
      ...
      const result = AuthSync.download();
  ...
  async backupToCloud(silent) {
    ...
    try {
      ...
      AuthSync.upload(data, deepseekKey);
      Store.set('_lastLocalSave', Date.now());
      this._lastSyncTime = new Date().toISOString();
  ...
  showLoginModal() {
    ...
    `, () => {
      ...
      errEl.style.display = 'none';
      try {
        AuthSync.signIn(email, password);
      } catch(e) {
        errEl.textContent = e.message; errEl.style.display = 'block';
        throw e; // 让 showModal 保持打开
      }
  ...
  showChangePasswordModal() {
    ...
    `, () => {
      ...
      errEl.style.display = 'none';
      try {
        AuthSync.changePassword(oldPwd, new1);
      } catch(e) {
        errEl.textContent = e.message; errEl.style.display = 'block';
        throw e;
      }
  ...
  _getCloudSyncHTML() {
    ...
    if (!loggedIn) {
      return `
        <div class="card">
          <div class="card-title"><span class="card-icon">🔐</span>账号同步</div>
          <div style="font-size:13px; color:var(--text-light); margin-bottom:12px;">
            注册/登录账号，数据将按账号保存<br>
            同一设备登录同账号即可恢复全部数据
          </div>
      ...
      <div class="card">
        ...
        <div style="display:flex; align-items:center; gap:10px; margin-bottom:12px;">
          ...
          <div>
            ...
            <div style="font-size:11px; color:var(--text-light);">数据按账号保存在本机</div>
