...
const DERMA_TREATMENTS = {
  '黄褐斑': [
    ...
    { name: '维A酸类', type: '外用', detail: '促进表皮更新，常与氢醌联用' },
    { name: '皮秒激光', type: '医美', detail: '755nm/1064nm皮秒，爆破黑色素颗粒' },
    { name: '强脉冲光IPL', type: '医美', detail: '宽谱光淡化色素，改善肤质' },
    { name: '化学剥脱', type: '医美', detail: '果酸/水杨酸剥脱表皮色素' },
    { name: '口服氨甲环酸', type: '口服', detail: '抑制纤溶酶，减少黑色素生成' },
  ],
  '雀斑': [
    { name: '强脉冲光IPL', type: '医美', detail: '首选治疗，光热作用破坏黑色素' },
    { name: 'Q开关激光', type: '医美', detail: '532nm/694nm精确爆破雀斑色素' },
    { name: '防晒', type: '防护', detail: '严格防晒防止复发，SPF50+PA++++' },
  ],
  '寻常痤疮': [
    { name: '过氧化苯甲酰', type: '外用', detail: '杀菌+溶解角质，轻中度痤疮一线' },
    { name: '维A酸类', type: '外用', detail: '阿达帕林/他扎罗汀，调节角化' },
    { name: '抗生素', type: '外用/口服', detail: '克林霉素/多西环素，需防耐药' },
    { name: '异维A酸', type: '口服', detail: '重度痤疮金标准，注意致畸和副作用' },
    { name: '果酸换肤', type: '医美', detail: '化学剥脱改善粉刺和痘印' },
    { name: '光动力疗法', type: '医美', detail: 'ALA-PDT杀灭痤疮丙酸杆菌' },
  ],
  '玫瑰痤疮': [
    { name: '甲硝唑凝胶', type: '外用', detail: '抗炎，改善红斑和丘疹' },
    { name: '伊维菌素乳膏', type: '外用', detail: '杀灭毛囊蠕形螨，抗炎' },
    { name: '溴莫尼定凝胶', type: '外用', detail: 'α受体激动剂，暂时收缩血管改善红斑' },
    { name: '多西环素', type: '口服', detail: '亚抗菌剂量，抗炎为主' },
    { name: '强脉冲光/IPL', type: '医美', detail: '封闭扩张毛细血管，改善潮红' },
    { name: '染料激光', type: '医美', detail: '585nm/595nm精确封闭血管' },
  ],
  '白癜风': [
    { name: '糖皮质激素', type: '外用/口服', detail: '抑制自身免疫反应' },
    { name: '钙调磷酸酶抑制剂', type: '外用', detail: '他克莫司/吡美莫司，面颈部适用' },
    { name: 'NB-UVB光疗', type: '光疗', detail: '窄谱中波紫外线，促进复色' },
    { name: '308nm准分子激光', type: '医美', detail: '靶向照射白斑部位' },
    { name: 'JAK抑制剂', type: '外用', detail: '鲁索替尼乳膏，FDA批准新型药物' },
  ],
  '银屑病': [
    { name: '糖皮质激素', type: '外用', detail: '轻中度首选，不同强度按部位选择' },
    { name: '维生素D3衍生物', type: '外用', detail: '卡泊三醇，调节角化' },
    { name: '甲氨蝶呤', type: '口服', detail: '中重度系统用药' },
    { name: '生物制剂', type: '注射', detail: 'TNF-α抑制剂/IL-17/IL-23抑制剂' },
    { name: 'NB-UVB光疗', type: '光疗', detail: '安全有效的物理治疗' },
  ],
};
    ...
    if (diseases.length === 0) {
      ...
      if (treatments.length > 0) {
        ...
        unique.forEach(t => {
          ...
          html += '<span style="font-size:10px;padding:2px 6px;border-radius:4px;background:#E8F5E9;color:#2E7D32;">'+t.type+'</span>';
          html += '</div>';
...
输出JSON格式：
{
  ...
  clearData() {
    if (confirm('确定要清空所有数据吗？此操作不可恢复。')) {
      ['profile','todos','habits','schedules','diaries','expenses','notes','health','customModuleData','dailyPlans','weeklyReports','aiRules','focusGuided','reminders','readings','fitnessLog','dermaKnowledge','dermaSearchHistory','pwaDataChecked'].forEach(k => Store.remove(k));
      this.loadProfile();
      this.loadAllData();
      this.renderTabbar();
      this.navigate('settings');
