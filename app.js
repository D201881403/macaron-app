...
const App = {
  ...
  _cycleDeductState(el) {
    const stateInput = document.getElementById('deductState');
    const current = stateInput?.value || 'none';
    let next = 'none', label = '✗ 未划扣 (0/3)';
    if (current === 'none') { next = 'all'; label = '✓ 全部已划扣 (3/3)'; }
    else if (current === 'all') { next = 'none'; label = '✗ 未划扣 (0/3)'; }
    // For existing 'some' state, cycle to all then none
    else { next = 'all'; label = '✓ 全部已划扣 (3/3)'; }
    if (stateInput) stateInput.value = next;
    el.textContent = label;
    el.classList.toggle('active', next === 'all');
  },
