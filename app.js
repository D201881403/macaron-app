...
// ============================================
// 账号同步模块 — 纯本地实现的账号系统
// 数据按账号存储在 localStorage，同一账号密码在同一浏览器可恢复数据
// ============================================

// 简单字符串哈希函数（非加密安全，仅用于本地存储隔离）
...
const AuthSync = {
  ...
  ACCOUNTS_KEY: 'macaron_accounts', // { [email]: { passwordHash, data: {...}, updatedAt } }
  SERVER_URL_KEY: 'macaron_server_url',
  ...
  // ---- 注册 ----
  // 返回 { token, user }；失败抛 Error
  async signUp(email, password) {
    email = (email || '').trim().toLowerCase();
    if (!email || !email.includes('@')) throw new Error('请输入有效邮箱');
    if (!password || password.length < 6) throw new Error('密码至少需要6位');

    // 远程模式
    if (this.isRemoteMode()) {
      const resp = await this._api('/api/register', {
        method: 'POST',
    ...
    // 本地模式
    const accounts = this._getAccounts();
    if (accounts[email]) throw new Error('该邮箱已注册，请直接登录');
    const passwordHash = _hashStr(password);
    accounts[email] = {
      passwordHash,
      data: null, // 首次注册无数据
    ...
    const token = _hashStr(email + ':' + password + ':' + Date.now());
    const user = { email };
  ...
  async signIn(email, password) {
    ...
    // 远程模式
    if (this.isRemoteMode()) {
      const resp = await this._api('/api/login', {
        method: 'POST',
        body: JSON.stringify({ email, password })
      });
      this.saveAuth(resp.token, resp.user);
    ...
    if (account.passwordHash !== _hashStr(password)) throw new Error('密码错误');
    const token = _hashStr(email + ':' + password + ':' + Date.now());
    const user = { email };
    this.saveAuth(token, user);
    return { token, user };
  ...
  async upload(appData, deepseekKey) {
    ...
    if (!user || !user.email) throw new Error('账号信息异常');

    // 远程模式
    if (this.isRemoteMode()) {
      const resp = await this._api('/api/upload', {
        method: 'POST',
        body: JSON.stringify({ data: appData, deepseekKey })
      });
      return { ok: true, updated_at: resp.updated_at };
    }
    ...
    account.data = appData || {};
    account.deepseekKey = deepseekKey || '';
    account.updatedAt = new Date().toISOString();
    accounts[user.email] = account;
    this._saveAccounts(accounts);
    return { ok: true, updated_at: account.updatedAt };
  },

  // ---- 下载数据（读取该账号的快照） ----
  // 返回 { data, updated_at, deepseek_key } 或抛错
  async download() {
    if (!this.isLoggedIn()) throw new Error('未登录');
    const user = this.getUser();
    if (!user || !user.email) throw new Error('账号信息异常');
    ...
    // 远程模式
    if (this.isRemoteMode()) {
      return await this._api('/api/download', { method: 'GET' });
    }
    ...
    return {
      ...
      deepseek_key: account.deepseekKey || ''
    };
  },
  ...
  async changePassword(oldPassword, newPassword) {
    if (!this.isLoggedIn()) throw new Error('未登录');
    if (!newPassword || newPassword.length < 6) throw new Error('新密码至少需要6位');
    const user = this.getUser();
    if (!user || !user.email) throw new Error('账号信息异常');

    // 远程模式
    if (this.isRemoteMode()) {
      await this._api('/api/change-password', {
        method: 'POST',
    ...
    const account = accounts[user.email];
    if (!account) throw new Error('账号不存在');
    if (account.passwordHash !== _hashStr(oldPassword)) throw new Error('当前密码错误');
    account.passwordHash = _hashStr(newPassword);
    accounts[user.email] = account;
    this._saveAccounts(accounts);
    // 更新 token
    const token = _hashStr(user.email + ':' + newPassword + ':' + Date.now());
    this.saveAuth(token, user);
    return { ok: true };
  },
  ...
  verifyToken() {
    ...
    const user = this.getUser();
    if (!user || !user.email) { this.logout(); return false; }
    // 远程模式：只检查 token 存在即可（实际有效性由 API 调用时验证）
    if (this.isRemoteMode()) return true;
    // 本地模式：检查账号是否存在
    const account = this._getAccount(user.email);
    if (!account) { this.logout(); return false; }
    return true;
  }
};
...
const DERMA_INDEX = [
  ...
  { cn: '术后护理指南', en: 'Post-procedure Care', tags: ['医美','术后','护理','激光','注射','深圳联合丽格'], cat: '医美', dermnet: '', baike: '', desc: '各类医美治疗术后护理详细指南（来源：深圳联合丽格医疗美容），涵盖染料激光、水光针、肉毒素、玻尿酸填充、点阵激光、光子嫩肤、调Q、皮秒、射频、光动力、激光脱毛等' },
];
...
const DERMA_TREATMENTS = {
  ...
  // ===== 术后护理指南（来源：深圳联合丽格医疗美容）=====
  '术后护理指南': [
    { name: '染料激光（595nm脉冲染料）术后护理', type: '术后护理', detail: '【深圳联合丽格】术后可能出现局部红肿，需立即冷敷，每次10-20秒间隙冷敷，重复多次，15-20分钟为一个阶段；红肿一般24-72小时自行消失。术后保持创面干燥，2天内严禁沾水，不可摩擦治疗面。如出现紫癜反应，属正常现象，一般一周左右自行消失，期间保持创面干燥清洁。严格预防日晒，外出需做好物理防晒（帽子、口罩、遮阳伞）并涂抹防晒霜。' },
    { name: '水光针术后护理', type: '术后护理', detail: '【深圳联合丽格】术后即刻敷医用敷贴，可额外冰敷15-20分钟减轻红肿。术后至少12小时内严禁沾水，可使用无菌喷雾清洁。3天内轻柔洗脸，勿用力揉擦，1周内避免高温洗烫（桑拿、温泉、热瑜伽等）。1周内仅使用温和医用护肤品，严禁使用含有美白、抗衰、祛痘等功效性化妆品。1周内避免过量烟酒及辛辣刺激食物。术后2-7天为黄金修复期，每天至少使用1次医用敷贴及医用修复产品（含透明质酸、胶原蛋白等）。严格防晒，外出撑伞戴帽，使用SPF30 PA+++以上防晒霜，尽量避免夏日正午外出。' },
    { name: '肉毒素注射术后护理', type: '术后护理', detail: '【深圳联合丽格】注射后现场观察20分钟，无不适方可离院。4-6小时内保持直立、避免平躺、避免剧烈运动或按摩注射区域，防止药物移位（尤其防止渗入眼球附近引起眼肌麻痹）。6小时后方可清洗注射部位。24小时内禁饮酒。2周内严禁使用氨基糖苷类抗生素（庆大霉素、妥布霉素、奈替米星、卡那霉素等，会加强肉毒素毒性）。1周内避免用力咀嚼硬物或做夸张表情。2周后回访评估效果。可能出现轻微头痛可服对乙酰氨基酚止痛；局部淤青可冰敷处理。' },
    { name: '玻尿酸填充术后护理', type: '术后护理', detail: '【深圳联合丽格】术后1-3天为肿胀高峰期，禁止用手触摸或揉搓填充部位以防移位。24小时内冰袋（裹毛巾）冷敷，每次15分钟间隔30分钟，消肿止痛。48小时后换40°C左右热敷促进吸收。1周内禁化妆（尤其粉底液，化学成分刺激伤口易感染）。避免剧烈运动（出汗带细菌进入针眼易发炎）。忌口辛辣、海鲜、牛羊肉等发物，饮食清淡+补充维生素C。晚11点前睡觉增强修复力。术后3天红肿可消退大半，3-5天基本恢复正常。若出现大理石样花纹、剧烈疼痛、视物模糊、头痛等症状，应立即就诊。' },
    { name: '剥脱性点阵激光（CO2/铒激光）术后护理', type: '术后护理', detail: '【深圳联合丽格】术后立即医用冰袋隔无菌纱布间隙冷敷，每次10-20秒，重复多次，15-20分钟一个阶段，显著减轻灼热刺痛。术后6-8小时内不沾水，之后可用生理盐水轻轻清洁。遵医嘱涂抹抗菌药膏（红霉素/莫匹罗星/夫西地酸等）预防感染，每日1-2次；可配合生长因子喷剂加速愈合（喷后15分钟再涂抗生素软膏，3-4小时后轻拭去，一日3-4次）。创面渗出结厚痂可用无菌生理盐水浸湿棉球轻拭。红肿1-2天消退，结痂5-14天脱落；痂皮脱落前严禁沾水、禁搓洗、禁摩擦抓挠、禁涂化妆品护肤品、禁用洗面奶。口周治疗1月内禁咀嚼硬食。脱痂后恢复护肤但需持续防晒SPF30 PA+++以上。术后2-3周内停用所有去角质产品（磨砂膏、化学焕肤、果酸等）。避免功能性产品（美白精华、视黄醇、高浓度活性成分）。至少2周避免饮酒、桑拿、汗蒸、热水浴、游泳池。' },
    { name: '非剥脱点阵激光术后护理', type: '术后护理', detail: '【深圳联合丽格】术后轻中度水肿性红斑约5天自行消退。可正常接触水分，建议用柔和氨基酸洁面清洁。1周内避免剧烈搓洗或卸妆动作。按医嘱使用医用保湿产品或保湿面膜修复皮肤屏障，选择含神经酰胺、角鲨烷等成分。持续物理防晒（帽子、口罩、遮阳伞）+SPF30 PA+++以上防晒霜。避免使用含酸类（果酸、水杨酸）、维A醇、酒精等刺激性成分的护肤品。' },
    { name: '强脉冲光/光子嫩肤术后护理', type: '术后护理', detail: '【深圳联合丽格】术后暂时性红斑一般2小时左右自行消退。部分色斑处可能出现薄痂，约1周脱落，切勿用手抓扣。术后一般可正常洗脸、护肤，需温和清洁，使用清水或氨基酸洗面奶洁面，禁用热水，切勿摩擦面部。红斑褪去或结痂脱落后方可化妆。对于皮肤敏感的患者术后应加强皮肤屏障修复，在医生指导下使用温和的医用保湿霜或保湿面膜。严格防晒，建议物理防晒（戴帽子、口罩或打伞）并使用SPF30 PA+++以上防晒霜。' },
    { name: '调Q激光术后护理', type: '术后护理', detail: '【深圳联合丽格】术后红肿及结痂5-7天逐渐脱落。痂皮脱落前严禁沾水，每日1-2次抗菌素软膏防感染（红霉素/莫匹罗星/夫西地酸等）。若创面渗出明显导致结痂较厚，可采用无菌生理盐水浸湿无菌棉球轻轻擦拭清洁。脱痂后可恢复正常洗脸化妆。需长期持续防晒，物理防晒+SPF30 PA+++以上防晒霜。' },
    { name: '皮秒激光术后护理', type: '术后护理', detail: '【深圳联合丽格】黄褐斑治疗：术后暂时性红斑约2小时消退，可正常洗脸护肤化妆，无停工期。嫩肤治疗：轻微至中度红斑、水肿或紫癜5-7天内自行消退，清水轻柔洗脸，加强屏障修复，使用医用保湿产品/面膜。轻微瘙痒可服抗组胺药（如西替利嗪）。长期物理防晒+SPF30 PA+++以上防晒霜。术后2-3周内避免使用含酸类、维A醇、酒精等刺激性成分的护肤品。' },
    { name: '射频紧肤（热玛吉/超声刀）术后护理', type: '术后护理', detail: '【深圳联合丽格】术后轻度暂时性红斑、水肿1-2天自行消退，无需特殊处理。可正常洗脸化妆。常规防晒即可。微针射频治疗护理同剥脱性激光（见上）。术后3天内禁高温环境（桑拿/热敷），避免剧烈运动。若不慎烫伤出现水泡，少量小水泡可用无菌纱布加生理盐水湿敷促进吸收；大水泡需到医院抽液；水泡破裂伴糜烂需用抗生素药膏或烫伤膏预防感染。' },
    { name: '光动力疗法（痤疮/鲜红斑痣）术后护理', type: '术后护理', detail: '【深圳联合丽格】痤疮ALA-PDT：术后2天内严格避光、避免强光源照射，室内常规照明无碍。持续使用SPF30 PA+++以上防晒霜。术后皮肤干涩需加强保湿，遵医嘱使用保湿型医学护肤品修复屏障。2-3天可能出现轻微水肿性红斑，可冷敷或冷喷缓解。可能出现小脓疱、薄痂皮或脱皮，1-2周自然恢复，无需特殊处理；脓疱较多时遵医嘱外用或口服抗生素。鲜红斑痣海姆泊芬光动力：术后2周内严格避光，外出需全面防护（太阳镜、宽边帽、长袖衣裤、手套）。前3天肿胀明显可冷喷或湿敷无菌生理盐水/蒸馏水。禁抓挠摩擦结痂处，每日1-2次抗生素软膏防感染。结痂约2周自然脱落。术后1-2天不适可适量服止痛药。' },
    { name: '激光脱毛术后护理', type: '术后护理', detail: '【深圳联合丽格】术后无水疱则当天可沐浴，禁用沐浴露/肥皂，水温不可过热。注意保湿滋养肌肤。四肢暴露区域需穿长袖长裤+SPF30 PA+++以上防晒霜。按疗程治疗，1-2月进行下一次，保证治疗效果。' },
    { name: '通用微创项目术后护理总则', type: '术后护理', detail: '【深圳联合丽格】1.清洁：无创类项目可沾水，建议术后一周内温凉水轻柔洗脸，适当用洗面奶，正常护肤。有创类项目少则几小时注射部位不沾水，多则结痂脱落前均不可沾生水。2.保湿：第一周每天用械字号医用修复面膜，第二周可隔天敷一次；选择含透明质酸、烟酰胺、葡聚糖、神经酰胺、泛醇等保湿修复类，少用或不用含酸、维A类产品。3.防晒：重中之重，避免术后反黑、色沉。有创类术后硬防晒为主（打伞戴帽戴口罩），待皮肤修复后使用防晒霜；无创类可涂抹防晒霜，注意补涂。4.生活：心情放松，适量运动少出汗，饮食清淡少辛辣，规律作息多饮水，术后至少2周禁烟酒、桑拿、温泉。' },
  ],
};
...
const App = {
    ...
    wrap.innerHTML = `
      ...
      <div class="card">
        ...
        <div style="display:flex; justify-content:space-around; text-align:center; gap:8px;">
          <div style="flex:1;">
            <div style="font-size:20px; font-weight:700; color:var(--primary-dark);">${this.todos.filter(t => !t.done).length}</div>
            <div style="font-size:10px; color:var(--text-light);">待办</div>
          </div>
          <div style="flex:1;">
            <div style="font-size:20px; font-weight:700; color:var(--success);">${this.getHabitDoneToday()}</div>
          ...
          <div style="flex:1;">
            ...
            <div style="font-size:10px; color:var(--text-light);">喝水</div>
          </div>
        </div>
      </div>
  ...
  showAddReadingModalById(id) { const r = this.readings.find(b => b.id === id); if(r) this.showAddReadingModal(r); },
  ...
  _renderCaseItem(c) {
    const deducted = c.deducted !== undefined ? c.deducted : (c.deductions && (c.deductions.a || c.deductions.b || c.deductions.c));
    return `
      <div class="case-item" onclick="App.editCase(${c.id})" style="cursor:pointer;">
        ...
        <div style="margin-top:6px;">
          <span class="case-tag ${deducted ? 'yes' : 'no'}">${deducted ? '✓ 已划扣' : '✗ 未划扣'}</span>
        </div>
  ...
  showAddCaseModal(existing) {
    const isEdit = !!existing;
    const c = existing || { date: this._dateToISO(new Date()), caseName: '', project: '', caseRecorded: false, consentForm: false, deducted: false, note: '' };
    const deducted = existing ? (existing.deducted !== undefined ? existing.deducted : (existing.deductions && (existing.deductions.a || existing.deductions.b || existing.deductions.c))) : false;
    ...
    this.showModal(isEdit ? '编辑案例' : '添加每日案例', `
      ...
      <div style="display:flex; gap:8px; margin-bottom:12px;">
        <div class="chip ${deducted ? 'active' : ''}" data-deducted="1" onclick="App._toggleDeductSingle(this)">💰 划扣</div>
      </div>
    ...
    `, () => {
      ...
      const isDeducted = document.querySelector('[data-deducted].chip.active') ? true : false;
  ...
  showFitnessDetail(filterType) {
    ...
    let h = '';
    if (ws.length>0) { h += \'<div class="field-label">⚖️ 体重记录（点击详情）</div><div class="card" style="padding:8px 12px;">\'+ws.map(w=>"<div onclick=\\"App.showFitnessItemDetail(\'weight\',\'"+w.date+"\')\\" style=\\"display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid var(--border);font-size:13px;cursor:pointer;\\"><span style=\\"color:var(--text-light);\\">"+w.date+"</span><span style=\\"font-weight:700;\\">"+w.value+"kg"+(w.bodyFat?" · 体脂"+w.bodyFat+"%":"")+"</span></div>").join("")+"</div>"; }
    if (ms.length>0) { h += \'<div class="field-label">📐 围度记录（点击详情）</div><div class="card" style="padding:8px 12px;">\'+ms.map(m=>"<div onclick=\\"App.showFitnessItemDetail(\'measurement\',\'"+m.date+"\')\\" style=\\"display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid var(--border);font-size:13px;cursor:pointer;\\"><span style=\\"color:var(--text-light);\\">"+m.date+"</span><span>胸"+(m.chest||"-")+" 腰"+(m.waist||"-")+" 臀"+(m.hip||"-")+"</span></div>").join("")+"</div>"; }
    if (wm.length>0) { h += '<div class=\"field-label\">🍽️ 本周饮食（点击详情）</div><div class=\"card\" style=\"padding:8px 12px;\">';const mn={breakfast:'🌅早',lunch:'☀️午',dinner:'🌙晚',snack:'🍪加'};h+=wm.map((m,idx)=>\"<div onclick=\\\"App.showFitnessItemDetail('meal',\"+idx+\")\\\" style=\\\"padding:6px 0;border-bottom:1px solid var(--border);font-size:13px;cursor:pointer;\\\"><div style=\\\"display:flex;justify-content:space-between;\\\"><span style=\\\"color:var(--text-light);\\\">\"+m.date+\" \"+(mn[m.type]||\"\")+\"</span><span style=\\\"font-weight:600;\\\">\"+(m.calories||\"?\")+\"kcal</span></div><div style=\\\"margin-top:2px;\\\">\"+m.food+\"</div></div>\").join(\"\");h+=\"</div>\"; }
    if (wsl.length>0) { h += '<div class=\"field-label\">😴 本周作息（点击详情）</div><div class=\"card\" style=\"padding:8px 12px;\">';const qn=['😴','😪','😊','😄'];h+=wsl.map(s=>\"<div onclick=\\\"App.showFitnessItemDetail('sleepLog','\"+s.date+\"')\\\" style=\\\"display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid var(--border);font-size:13px;cursor:pointer;\\\"><span style=\\\"color:var(--text-light);\\\">\"+s.date+\"</span><span>\"+s.sleepTime+\"-\"+s.wakeTime+\" (\"+s.hours+\"h) \"+qn[s.quality-1]+\"</span></div>\").join(\"\");h+=\"</div>\"; }
    if (h==='') h = '<div style="text-align:center;padding:20px;color:var(--text-light);">暂无记录</div>';
    this._fitnessDetailCache = { weights: ws, measurements: ms, meals: wm, sleepLog: wsl };
    ...
    wrap.innerHTML = `
      ...
      <div class="card">
        ...
        <div style="display:flex; flex-direction:column; gap:10px;">
          ...
          <button class="btn btn-ghost" onclick="App.importData()">📥 导入数据</button>
  ...
  importData() {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        if (!confirm('⚠️ 导入将覆盖当前所有本地数据，确定继续？')) return;
        
        const keys = [
          'profile', 'todos', 'habits', 'schedules', 'diaries', 
        ...
        this.renderTabbar();
        this.navigate('settings');
        this.toast('✅ 数据导入成功，共恢复 ' + keys.filter(k => data[k] !== undefined).length + ' 项');
      } catch (err) {
        this.toast('❌ 导入失败：' + (err.message || '未知错误'));
      }
    };
...
输出JSON格式：
{
  ...
  showModal(title, bodyHTML, onConfirm) {
    ...
    // 支持 async onConfirm：返回 Promise 时，等其 resolve 再关闭；reject 则保持弹窗打开
  ...
  renderAttendanceMonth() {
    ...
    return `
      <div class="card" style="text-align:center; padding: 16px;">
        ...
        <div style="font-size:11px; color:var(--text-muted); margin-top:6px;">点击月历中某天可手动修改打卡时间</div>
      </div>
...
END:VCALENDAR`;
  ...
  async restoreFromCloud(isFreshLogin) {
    try {
      if (isFreshLogin) this.toast('☁️ 正在恢复账号数据...');
      const result = await AuthSync.download();
      if (!result || !result.data) {
        // 账号下暂无快照，把当前本地数据上传一份
        if (isFreshLogin) {
          setTimeout(() => this.backupToCloud(true), 300);
          this.toast('☁️ 账号暂无备份，已保存当前数据');
        }
        return;
      }
      ...
      if (!isFreshLogin) {
        ...
        const cloudTime = result.updated_at ? new Date(result.updated_at).getTime() : 0;
      ...
      console.error('账号恢复失败:', e);
      if (isFreshLogin) this.toast('⚠️ 账号恢复失败: ' + e.message);
    }
  },
  ...
  async backupToCloud(silent) {
    ...
    try {
      ...
      Store.set('_lastLocalSave', Date.now());
      this._lastSyncTime = new Date().toISOString();
      if (!silent) this.toast('✅ 数据已备份到账号');
    } catch(e) {
      console.error('账号备份失败:', e);
    ...
    this.showModal('📝 注册账号', `
      <div style="font-size:13px; line-height:1.8; color:var(--text);">
        ...
        <div style="font-size:11px; color:var(--text-muted); margin-top:8px; line-height:1.6;">
          💡 账号数据保存在本机浏览器中，同一设备登录同账号可恢复数据。换设备请用「导出/导入」迁移。
        </div>
      </div>
    `, () => {
      ...
      if (!email || !email.includes('@')) { errEl.textContent = '请输入有效邮箱'; errEl.style.display = 'block'; throw new Error('验证失败'); }
      if (!p1 || p1.length < 6) { errEl.textContent = '密码至少需要6位'; errEl.style.display = 'block'; throw new Error('验证失败'); }
      if (p1 !== p2) { errEl.textContent = '两次密码不一致'; errEl.style.display = 'block'; throw new Error('验证失败'); }
      errEl.style.display = 'none';
      return this._doSignUp(email, p1);
    });
  },
  ...
  async _doSignUp(email, password) {
    const errEl = document.getElementById('regError');
    try {
      await AuthSync.signUp(email, password);
    } catch(e) {
      if (errEl) { errEl.textContent = e.message; errEl.style.display = 'block'; }
      throw e; // 让 showModal 保持打开
    }
    this.toast('✅ 注册成功！正在保存数据...');
    setTimeout(() => this.backupToCloud(true), 300);
    setTimeout(() => { this.navigate('settings'); this.toast('✅ 已登录：' + email); }, 800);
  },
  ...
  showLoginModal() {
    ...
    `, () => {
      ...
      if (!email || !password) { errEl.textContent = '请输入邮箱和密码'; errEl.style.display = 'block'; throw new Error('验证失败'); }
      errEl.style.display = 'none';
      return this._doSignIn(email, password);
    });
  },
  ...
  async _doSignIn(email, password) {
    ...
    this.toast('✅ 登录成功：' + email);
    // 登录后从账号快照恢复数据
    setTimeout(() => this.restoreFromCloud(true), 300);
  },
  ...
  showChangePasswordModal() {
    ...
    `, () => {
      ...
      if (!oldPwd) { errEl.textContent = '请输入当前密码'; errEl.style.display = 'block'; throw new Error('验证失败'); }
      if (!new1 || new1.length < 6) { errEl.textContent = '新密码至少需要6位'; errEl.style.display = 'block'; throw new Error('验证失败'); }
      if (new1 !== new2) { errEl.textContent = '两次密码不一致'; errEl.style.display = 'block'; throw new Error('验证失败'); }
      errEl.style.display = 'none';
      errEl.style.display = 'none';
      return this._doChangePassword(oldPwd, new1);
    });
  },

  async _doChangePassword(oldPassword, newPassword) {
    const errEl = document.getElementById('chpwdError');
    try {
      await AuthSync.changePassword(oldPassword, newPassword);
    } catch(e) {
      if (errEl) { errEl.textContent = e.message; errEl.style.display = 'block'; }
      throw e;
    }
    this.toast('✅ 密码已更新');
    setTimeout(() => this.navigate('settings'), 500);
  },
  ...
  _getCloudSyncHTML() {
    ...
    const serverUrl = AuthSync.getServerUrl();
    const isRemote = AuthSync.isRemoteMode();
    const modeLabel = isRemote ? '☁️ 远程服务器' : '💾 本地存储';
    const modeDesc = isRemote ? `服务器: ${serverUrl}` : '数据保存在本机浏览器';

    const serverConfig = `
      <div style="margin-top:12px; padding-top:10px; border-top:1px solid var(--border);">
        <div style="font-size:11px; color:var(--text-light); margin-bottom:6px;">⚙️ 同步模式: <b>${modeLabel}</b></div>
        <div style="font-size:10px; color:var(--text-muted); margin-bottom:8px;">${modeDesc}</div>
        <button class="chip" style="width:100%; justify-content:center;" onclick="App.showServerConfigModal()">⚙️ 服务器设置</button>
      </div>`;

    if (!loggedIn) {
      return `
        <div class="card">
          <div class="card-title"><span class="card-icon">🔐</span>账号同步 <span style="font-size:10px; color:var(--text-muted);">[${modeLabel}]</span></div>
          <div style="font-size:13px; color:var(--text-light); margin-bottom:12px;">
            ${isRemote ? '远程模式：数据同步到服务器，跨设备恢复' : '注册/登录账号，数据将按账号保存'}<br>
            ${isRemote ? '换设备登录同账号即可恢复全部数据' : '同一设备登录同账号即可恢复全部数据'}
          </div>
          <div style="display:flex; gap:8px;">
    ...
    return `
      <div class="card">
        <div class="card-title"><span class="card-icon">🔐</span>账号同步 <span style="font-size:11px;color:#27ae60;">✅ 已登录</span></div>
        <div style="display:flex; align-items:center; gap:10px; margin-bottom:12px;">
          <div style="font-size:28px;">👤</div>
          <div>
            <div style="font-weight:700;">${user ? user.email : '已登录'}</div>
            <div style="font-size:11px; color:var(--text-light);">${isRemote ? '数据同步到远程服务器' : '数据按账号保存在本机'}</div>
            ${lastSync ? `<div style="font-size:10px; color:var(--text-muted);">上次同步: ${lastSync}</div>` : ''}
          </div>
        </div>
        <div style="display:flex; gap:8px;">
          <button class="btn btn-ghost" style="flex:1;" onclick="App.backupToCloud()">🔄 保存到账号</button>
          <button class="btn btn-ghost" style="flex:1;" onclick="App.restoreFromCloud()">📥 从账号恢复</button>
        </div>
  ...
  // 退出登录
  logoutCloud() {
    if (confirm('退出登录后本地数据保留，账号快照不会删除。确定退出？')) {
      AuthSync.logout();
  ...
  showServerConfigModal() {
    document.querySelectorAll('.modal-overlay').forEach(el => el.remove());
    const currentUrl = AuthSync.getServerUrl();
    this.showModal('⚙️ 服务器设置', `
      <div style="font-size:13px; line-height:1.8; color:var(--text);">
        <div style="background:#f5f5f5; border-radius:8px; padding:10px; margin-bottom:12px; font-size:12px; color:var(--text-light);">
          <b>💡 说明：</b><br>
          • <b>本地模式</b>（默认）：数据存在浏览器，删除主屏幕图标会丢失<br>
          • <b>远程模式</b>：填写服务器地址后，数据同步到云端，跨设备恢复<br>
          • 需自行部署 Docker 后端（见 server 目录）
        </div>
        <div class="field-label">服务器地址</div>
        <input class="input" id="serverUrlInput" type="text" placeholder="如 https://my-server.com" value="${currentUrl}" autocomplete="off">
        <div style="font-size:11px; color:var(--text-muted); margin-top:6px;">留空 = 本地存储模式；填写 = 远程服务器模式</div>
        <div id="serverTestResult" style="font-size:12px; margin-top:8px; display:none;"></div>
      </div>
    `, () => {
      const url = document.getElementById('serverUrlInput').value.trim();
      AuthSync.setServerUrl(url);
      if (url) {
        this.toast('✅ 已切换到远程模式：' + url);
      } else {
        this.toast('✅ 已切换到本地存储模式');
      }
      setTimeout(() => this.navigate('settings'), 500);
    });
    // 添加测试按钮
    setTimeout(() => {
      ...
      if (modalBody) {
        ...
        testBtn.style.cssText = 'width:100%; justify-content:center; margin-top:10px;';
        ...
        testBtn.onclick = async () => {
          ...
          if (!url) { resultEl.textContent = '⚠️ 请先填写地址'; resultEl.style.color = '#E17055'; resultEl.style.display = 'block'; return; }
          ...
          try {
            ...
            const data = await resp.json();
            if (data.ok) {
              resultEl.textContent = '✅ 连接成功！服务正常';
